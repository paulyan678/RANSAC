...
class LinearRegressor():
    ...
    def predict(self, x):
        A = np.vstack([x, np.ones(len(x))]).T
        return (A @ self.w).flatten()

def model_error_metric(y_preds, y_datas):
    return np.sum((y_preds - y_datas)**2) / len(x)

def supporter_metric(y_pred, y_data):
...
if __name__ == "__main__":
    ...
    ransac = RANSAC(k=100, 
                    ...
                    d=20, 
                    t=0.05, 
                    supporter_metric=supporter_metric,
                    model_error_metric=model_error_metric)
    ransac.fit(x, y)

    plt.scatter(x, y)
    plt.plot(x, ransac.best_model.predict(x), color='red')
    plt.show()